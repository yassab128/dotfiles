...
main()
{
	...
	*pattern = 0;
	...
	items_number = ARRAY_SIZE(choices);
	...
	while (g_keep_going) {
		...
		case 'k':
			...
			break;
		case '\n':
			chosen_option = choices[current_item_index];
			...
			def_prog_mode();
			WARNIF(endwin() == ERR);
			spawn_external_process(chosen_option);
			reset_prog_mode();

			sa.sa_handler = clean_exit;
			WARNIF(sigaction(SIGINT, &sa, 0) == -1);

			if (wgetch(menu_win) == ERR) {
				WARNIF(refresh() == ERR);
				g_keep_going = 0;
			}
			continue;
